...
h3k4me3 <- Colname(read.delim("HistoneEsb4H3k4me3ME0.bed", header = F, stringsAsFactors = F))
h3k9ac <- Colname(read.delim("HistoneEsb4H3k09acME0.bed", header = F, stringsAsFactors = F))
h3k9me3 <- Colname(read.delim("HistoneEsb4H3k09me3ME0.bed", header = F, stringsAsFactors = F))
h3k27ac <- Colname(read.delim("HistoneEsb4H3k27acME0.bed", header = F, stringsAsFactors = F))
h3k27me3 <- Colname(read.delim("HistoneEsb4H3k27me3ME0.bed", header = F, stringsAsFactors = F))
h3k36me3 <- Colname(read.delim("HistoneEsb4H3k36me3ME0.bed", header = F, stringsAsFactors = F))
ctcf <- Colname(read.delim("TfbsEsb4CtcfME0.bed", header = F, stringsAsFactors = F))
pol2 <- Colname(read.delim("TfbsEsb4Pol2ME0.bed", header = F, stringsAsFactors = F))
p300 <- Colname(read.delim("TfbsEsb4P300ME0.bed", header = F, stringsAsFactors = F))

#--- Segment ENCODE broadPeak
marks.bi <- cbind(Binary(h3k4me1), Binary(h3k4me3), Binary(h3k9ac), Binary(h3k9me3),
  Binary(h3k27ac), Binary(h3k27me3), Binary(h3k36me3), Binary(ctcf), Binary(pol2), Binary(p300))
marks.si <- cbind(Signal(h3k4me1), Signal(h3k4me3), Signal(h3k9ac), Signal(h3k9me3),
  Signal(h3k27ac), Signal(h3k27me3), Signal(h3k36me3), Signal(ctcf), Signal(pol2), Signal(p300))
colnames(marks.si) <- colnames(marks.bi) <- c("k4me1", "k4me3", "k9ac", "k9me3", "k27ac", "k27me3", "k36me3", "ctcf", "pol2", "p300")
...
esb4.df <- data.frame(row.names = rownames(refgene.tx), rna.df[-idx, ],
...
esb4.df[is.na(esb4.df)] <- 0
save(marks.bi, esb4.df, file = "~/Dropbox/Network/R1/esb4Df.rdt")  # ESB4
